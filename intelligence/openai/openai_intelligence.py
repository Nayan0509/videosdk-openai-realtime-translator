...
class OpenAIIntelligence:
    ...
    async def send_request(self, request: ClientToServerMessage):
        ...
        try:
            req_type = getattr(request, 'type', 'unknown')
        except Exception:
            req_type = 'unknown'
    ...
    async def send_audio_data(self, audio_data: bytes):
        ...
        print(f"[OpenAI] send: audio bytes={len(audio_data)}")
            ...
            async for response in self.ws:
                try:
                    ...
                    if response.type == aiohttp.WSMsgType.TEXT:
                        print(f"[OpenAI] recv: TEXT bytes={len(response.data)}")
